    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 pb-28 md:pb-8">
        ...
        <div className="bg-white p-5 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
          <Link
            href="/student"
            className="min-h-[36px] inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 mb-4 transition"
          >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 mt-1">{subject.name}</h1>
          ...
          {topics.length === 0 ? (
            ...
            topics.map((topic, idx) => {
              ...
              return (
                <div
                  key={topic.id}
                  className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 shadow-sm space-y-4"
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-sm sm:text-base font-bold text-slate-900 truncate">{topic.name}</h4>
                      </div>
                      <p className="text-xs text-slate-500 mt-1 sm:ml-8 leading-relaxed">{topic.description}</p>
                    </div>

                    {topicQuiz && (
                      <Link
                        href={`/student/assessments/${topicQuiz.id}`}
                        className="min-h-[44px] px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-xl inline-flex items-center justify-center gap-1.5 transition self-start sm:self-auto shrink-0 shadow-sm"
                      >
                  ...
                  {topicLessons.length > 0 && (
                    <div className="sm:ml-8 pt-3 border-t border-slate-100 space-y-2">
                      ...
                      {topicLessons.map((item: LearningContent) => (
                        <div
                          key={item.id}
                          className="min-h-[48px] p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            <FileText className="w-4 h-4 text-indigo-600 shrink-0" />
                            <span className="text-xs font-semibold text-slate-800 break-words">{item.title}</span>
                          </div>
                          <Link
                            href={`/student/revision?topic=${topic.id}`}
                            className="min-h-[36px] text-xs text-indigo-600 hover:text-indigo-800 font-medium inline-flex items-center gap-1 shrink-0 self-start sm:self-auto"
                          >
